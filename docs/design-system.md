...
- **Ink is the emphatic surface** — solid, dark, confident. Used for primary actions and headings, never for amber's job.
...
| `app/components/ui/*.vue` | The nine shared components (auto-imported with the `Ui` prefix) |
...
| Scale | Role |
|-------|------|
| `ink` | Primary actions and headings. Near-black with a faint blue cast |
...
```
color="primary"   → ink    (actions, headings)
...
2. **Amber means a mention happened.** Not "warning", not "new", not "highlight
   this CTA". If the thing being coloured is not a mention, a tally or an active/live state, it is
   not amber.
3. **Green and red are market outcome and trend.** A saved-successfully toast is not green. Reserve
   `color="error"` for a state the user must actually act on.
...
- The **true accent is untouched**. The tally, the `<mark>` wash, live states and inline figures
  ...
  `text-mark-600 dark:text-mark-400` (see `transcripts/[id].vue`) — not `text-secondary`.
...
Used in the header, the footer, and on `/login`.
...
### `<UiUpsellBanner>` — **removed**

The paywall is gone: the site is a free, anonymous transcript archive. `UpsellBanner.vue` was
deleted along with `/pricing`, `/account`, `/signup` and the public markets pages. Do not
reintroduce a gate prompt — there is nothing to gate.
