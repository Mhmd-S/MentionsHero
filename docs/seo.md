...
|----------|---------|-------|
| `OgImageDefault.vue` | `/`, `/blog`, `/transcripts/[id]` | None |
| `OgImagePersona.vue` | `/personas/[slug]` | `name`, `description`, `imageUrl` |
| `OgImageBlog.vue` | `/blog/[...slug]` | `title`, `description`, `date` |

`/login` deliberately has no OG image — it is `noindex, nofollow`.
...
- **Excluded**: `/admin/**`, `/login`

The `/api/public/sitemap-urls` endpoint (`backend/routers/public.py`) emits `/personas/{slug or id}`
and nothing else. **Most personas have no slug**, so the id fallback is what keeps them in the
sitemap — filtering on slug alone drops nearly all of them.
...
### Gotcha: `/login` inherits nothing

The page sets `definePageMeta({ layout: false })` — it renders its own shell rather than the public
header/footer. That means it does **not** inherit the `Organization` + `WebSite` schema defined in
`app/layouts/default.vue`. This is fine:
...
Generated by `@nuxtjs/robots`. Disallows `/admin/` and `/login`.
...
- Non-indexed pages (`/login`, `/transcripts/[id]`): `robots: 'noindex, nofollow'`. `/transcripts/[id]` is `noindex` as a holdover from the paywall era — revisit it now that every transcript is free
