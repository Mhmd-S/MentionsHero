...
async def get_channel_videos(url: str) -> "ChannelInfo":
    ...
    if proc.returncode != 0:
        error_msg = stderr.decode().strip() if stderr else "Failed to fetch channel info"
        raise ValueError(error_msg)
    ...
    for line in lines:
        ...
        if not channel_id:
            channel_id = data.get("channel_id") or data.get("playlist_id") or ""
            channel_title = data.get("channel") or data.get("uploader") or data.get("playlist_title") or ""
