...
import { useTrading, type TradingSession, type SessionDetail, type TradingMarket, type ChannelMonitorStatus } from '~/composables/useTrading'

const { personas, fetchPersonas } = usePersonas()
const { fetchAllSeries } = usePolymarket()
const { getActiveSession, getSessionHistory, startSession, stopSession, getSessionDetail, streamSession, getMarketsForSeries, getChannelMonitorStatus } = useTrading()
...
// Market selection state (replaces event selection)
...
async function handleStart() {
  ...
  try {
    ...
    // If all active markets selected (or none explicitly chosen), send empty array = all
    ...
    if (result) {
      ...
      // Fetch initial detail then start streaming
...
// Reset series and markets when persona changes
...
// Load markets when series changes
watch(selectedSeriesId, async (newId) => {
  ...
  try {
    ...
    if (result) {
      ...
      // Default: select all active markets with search terms
