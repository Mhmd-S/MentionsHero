...
// Sync paywall toggles when transcript loads
watch(transcript, (newTranscript) => {
  if (newTranscript) {
    isPublic.value = newTranscript.is_public ?? false
    isPremium.value = newTranscript.is_premium ?? false
  }
})
...
async function updateVisibility() {
  ...
  try {
    await authFetch(`/api/transcripts/${route.params.id}`, {
      method: 'PATCH',
      body: { is_public: isPublic.value, is_premium: isPremium.value },
    })
  } catch (err) {
