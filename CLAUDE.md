...
- **PostgREST caps unbounded selects at 1000 rows, silently**: no error, the response just stops. Whole-table reads in `backend/services/public_service.py` go through `_select_all()`, which pages. An unpaged read of `transcript_speakers` (1073 rows) returned 1000 and made 12 healthy transcripts look broken
- **Only personas with transcripts are listed**: `get_public_personas()` filters to personas having at least one public transcript and attaches `transcript_count`. The homepage and the sitemap both come from it, so empty personas are neither shown nor submitted to search engines; their pages still resolve by direct URL
- **No speaker links, no public page**: `get_public_transcript()` returns `None` (→404) for a transcript with no `transcript_speakers` rows. Persona listings and keyword search already reach transcripts *through* speakers, so serving it directly only created an orphan URL with no persona breadcrumb
